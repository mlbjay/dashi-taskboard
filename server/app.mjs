...
function actorFromRequest(request) {
  const rawAgentId = requestHeader(request, "x-taskboard-agent-id");
  const rawAgentName = requestHeader(request, "x-taskboard-agent-name");
  if (rawAgentId !== undefined || rawAgentName !== undefined) {
    if (rawAgentId === undefined || rawAgentName === undefined) {
      throw new ApiError(400, "INVALID_ACTOR", "Agent identity requires both an ID and name");
    }
    const id = stringField(rawAgentId, "X-Taskboard-Agent-Id", { required: true, maxLength: 96 });
    if (!/^[a-z0-9](?:[a-z0-9._-]*[a-z0-9])?$/.test(id)) {
      throw new ApiError(400, "INVALID_ACTOR", "Agent ID contains unsupported characters");
    }
    let decodedName;
    try {
      decodedName = decodeURIComponent(rawAgentName);
    } catch {
    ...
    let avatarUrl = null;
    const rawAvatarUrl = requestHeader(request, "x-taskboard-agent-avatar");
    if (rawAvatarUrl !== undefined) {
      const value = stringField(rawAvatarUrl, "X-Taskboard-Agent-Avatar", { required: true, maxLength: 2048 });
      let parsed;
      try {
        parsed = new URL(value);
      } catch {
        throw new ApiError(400, "INVALID_ACTOR", "Agent avatar URL is invalid");
      }
      if (!["http:", "https:"].includes(parsed.protocol)) {
        throw new ApiError(400, "INVALID_ACTOR", "Agent avatar URL must use HTTP or HTTPS");
      }
